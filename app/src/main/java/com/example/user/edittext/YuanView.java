...
public class YuanView extends View {
    private static final int BACKGROUND_COLOR = 0x50555555;//默认背景颜色
    ...
    public YuanView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.YuanView);//自定义属性
        backgroundColor = ta.getColor(R.styleable.YuanView_background_color, BACKGROUND_COLOR);
        borderWidth = ta.getDimension(R.styleable.YuanView_border_width,
                BORDER_WIDTH);
        borderColor = ta.getColor(R.styleable.YuanView_border_color,
                BORDER_COLOR);
        text = ta.getString(R.styleable.YuanView_text);
        if (text == null) {//默认文字
            text = TEXT;
        }
        textSize = ta.getDimension(R.styleable.YuanView_text_size,
                TEXT_SIZE);//默认字体大小
        textColor = ta.getColor(R.styleable.YuanView_text_color,
                TEXT_COLOR);//默认字体颜色
    ...
    public void start() {
        ...
        countDownTimer = new CountDownTimer(3600, 36) {
            ...
            public void onTick(long millisUntilFinished) {
                progress = ((3600 - millisUntilFinished) / 3600f) * 360;//百分比*360°
